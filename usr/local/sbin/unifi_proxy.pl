      ...
      foreach my $siteObj (@{$siteList}) {
         ...
         if (! $gC->{'key'}) {
            ...
            if (ACT_DISCOVERY eq $gC->{'action'}) {
               ...
               addToLLD($gC, $siteObj, $parentObj, $objList, $selectingResult->{'data'}) if ($objList);
            } else {
            ...
            if (ACT_DISCOVERY eq $gC->{'action'}) {
               ...
               foreach (@{$objList}) {
                  ...
                  addToLLD($gC, $siteObj, $parentObj, $wrkSelectingResult->{'data'}, $selectingResult->{'data'}) if (@{$wrkSelectingResult->{'data'}} > 0);
